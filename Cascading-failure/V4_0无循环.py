...
# graph1 = nx.read_adjlist(r'C:\Users\Ludwigia\Desktop\实验数据\BA-pare\lambda4.5\graph1.adjlist')
graph1 = nx.read_adjlist(r'graph1.adjlist')
print('g1读取完毕:%s'%time.strftime("%a %b %d %H:%M:%S %Y", time.localtime()))

# graph2 = nx.read_adjlist(r'C:\Users\Ludwigia\Desktop\实验数据\BA-pare\lambda4.5\graph2.adjlist')
graph2 = nx.read_adjlist(r'graph2.adjlist')
print('g2读取完毕：'+'my name2:%s'%time.strftime("%a %b %d %H:%M:%S %Y", time.localtime()))

# G = nx.read_adjlist(r'C:\Users\Ludwigia\Desktop\实验数据\BA-pare\lambda4.5\G.adjlist')
G = nx.read_adjlist(r'G.adjlist')
...
try:
    ...
    ws.title = 'Sheet1'
# 创建数据帧
df = pd.DataFrame({'perA': [perA], 'perB': [perB]})
# 将数据帧追加到工作表中
